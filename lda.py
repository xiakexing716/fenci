...
#随机先分配类型，为每个文档中的各个单词分配主题
    ...
    #print'true'
        ...
        return topic
    def est(self):
        #Consolelogger.info(u"迭代次数为%s次"%self.iter_times)
    ...
    #保存theta文章-主题分布
        ...
        with codecs.open(self.paramfile,'w','utf-8') as f:
            ...
            f.write(u'每个类的高频词显示个数top_words_num='+str(self.top_words_num)+'\n')
        #保存每个主题topic的词
...
def preprocessing():
    ...
    for line in docs:
        if line != "":
            ...
            doc = Document()
            for item in tmp:
                if dpre.word2id.get(item):#已有的话，只是当前文档追加
